...
const [imageToDelete, setImageToDelete] = useState(null);
    ...
    const fetchData = async () => {
      try {
        ...
        const [projectsResponse, locationsResponse, templatesResponse, usersResponse] = await Promise.all([
          axios.get(`http://localhost:4000/api/project/designEngineer`, {
            headers: { Authorization: `Bearer ${user.token}` },
          }),
  ...
  const handleEndProject = async (projectId) => {
    try {
      const response = await axios.patch(
        `http://localhost:4000/api/api/project/${projectId}/end`,
        {},
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>Project DesignEngineer</TableCell>
            ...
            <DialogContent dividers>
              ...
              <Accordion>
                ...
                <AccordionDetails>
                  ...
                  <Typography><strong>Project designEngineer:</strong> {selectedProject.designEngineer || 'N/A'}</Typography>
                  <Typography>
              ...
              {selectedProject.bom && selectedProject.bom.categories.length > 0 ? (
                <>
                  ...
                  <Button variant="contained" color="secondary" onClick={() => handleGenerateBOMPDF('designEngineer')} sx={{ mt: 2 }}>
